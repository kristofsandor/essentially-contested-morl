...
class ECCEnvelope(MOPolicy, MOAgent):
    ...
    def get_q_sets(self, obs):
        ...
        q_set = th.einsum(
            "i,ikar->kar",
            th.tensor(self._episode_interp_weight).float().to(self.device),
            q_set,
    ...
    def _marginal_hv_sum(self) -> float:
        ...
        *after* appending to ``_episode_mo_returns_per_interp``.
        ...
        for returns in self._episode_mo_returns_per_interp:
            ...
            if len(returns) > 1:
                front_without = list(get_non_dominated({tuple(r) for r in returns[:-1]}))
    ...
    def ucb_best_weight_and_interp(self) -> tuple:
        ...
        if n_episodes < self.ucb_warmup_episodes:
            return (
                random_weights(self.net_reward_dim, 1, dist="dirichlet", rng=self.np_random),
                random_weights(self.num_interps, 1, dist="dirichlet", rng=self.np_random),
            )
        ...
        cand_w = random_weights(self.net_reward_dim, self.ucb_n_candidates, dist="dirichlet", rng=self.np_random)
        cand_iw = random_weights(self.num_interps, self.ucb_n_candidates, dist="dirichlet", rng=self.np_random)
        cand_keys = np.concatenate([cand_w, cand_iw], axis=1)  # [C, net_reward_dim + num_interps]
    ...
    def train(
        ...
        if weight is not None:
            w = np.array(weight)
            self._episode_interp_weight = np.array(self.interp_weight, dtype=np.float32)
        elif self.use_hv:
            w, iw = self.ucb_best_weight_and_interp()
            self._episode_interp_weight = iw.astype(np.float32)
        else:
            w = random_weights(
        ...
        for _ in range(1, total_timesteps + 1):
            ...
            if terminated or truncated:
                ...
                if "episode" in info.keys():
                    ...
                    per_interp_return = flat_return.reshape(self.num_interps, self.net_reward_dim)
                    for i in range(self.num_interps):
                        self._episode_mo_returns_per_interp[i].append(per_interp_return[i].copy())
