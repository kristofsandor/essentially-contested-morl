...
class QNet(nn.Module):
    ...
    def __init__(self, obs_shape, action_dim, rew_dim, num_interp, net_arch, drop_rate=0.01, layer_norm=True):
        ...
        Args:
            ...
            num_interp: The number of interpretations.
...
class ECCGPIPD(MOPolicy, MOAgent):
    ...
    @th.no_grad()
    def _rollout_dynamics(self, w: th.Tensor, interp_w: th.Tensor):
        # Dyna Planning
    ...
    def _interp_w_like(self, w: th.Tensor, interp_w=None) -> th.Tensor:
        """Broadcast a single interp weight to match the leading dims of ``w``.

        Returns a tensor of shape ``w.shape[:-1] + (num_interp,)`` so it can be fed to
        ``QNet.forward`` alongside ``w``. Defaults to the fixed eval reference weight.
        """
        if interp_w is None:
    ...
    def set_eval_interp_weight(self, interp_w) -> None:
        ...
        e.g. one-hot ``[1, 0]`` for deontological (row 0) or ``[0, 1]`` for
        utilitarian (row 1). Affects the agent's own ``eval`` only; the projected
    ...
    def update(self, weight: th.Tensor):
        ...
        for g in range(self.gradient_updates if self.global_step >= self.dynamics_rollout_starts else 1):
            ...
            # what makes the conditioned policy depend on interp_w.
            s_rewards = th.einsum(
                "ni,nir->nr",
                interp_w,
    ...
    def eval(self, obs: np.ndarray, w: np.ndarray, interp_w: Optional[np.ndarray] = None) -> int:
        ...
        callers with a fixed ``eval(obs, w)`` signature (morl_baselines' GPI-LS /
        ``eval_mo``) still work; pass it explicitly to query a specific interpretation.
        """
        obs = th.as_tensor(obs).float().to(self.device)
        w = th.as_tensor(w).float().to(self.device)
        if interp_w is None:
            interp_w = self._eval_interp_w
        iw = th.as_tensor(interp_w).float().to(self.device)
        for q_net in self.q_nets:
            q_net.eval()
        if self.use_gpi:
            action = self.gpi_action(obs, w, include_w=False, interp_w=iw)
        else:
            action = self.max_action(obs, w, interp_w=iw)
        for q_net in self.q_nets:
            q_net.train()
        return action

    def _act(self, obs: th.Tensor, w: th.Tensor, interp_w: th.Tensor) -> int:
        if self.np_random.random() < self.epsilon:
            return self.env.action_space.sample()
        else:
            if self.use_gpi:
                action, policy_index = self.gpi_action(
                    obs, w, return_policy_index=True, interp_w=interp_w
                )
                self.police_indices.append(policy_index)
                return action
            else:
                return self.max_action(obs, w, interp_w=interp_w)
        ...
        for i in range(num_batches):
            ...
            if self.gpi_pd:
                max_next_q, _ = self._envelope_target(
                    next_obs, w_b, th.stack(self.weight_support), iw_b
                )
            else:
                next_q_values = self.q_nets[0](next_obs, w_b, iw_b)
    ...
    def train_iteration(
        ...
        # exploration interp weight, sampled per episode and passed around like tensor_w
        ...
        def _scalarize_episode_return(episode_return: np.ndarray, scalar_weight: np.ndarray) -> float:
            ...
            matrix. Logging needs a 2D return in the policy-weight space, so collapse
            the shared task column once and average the interpretation-specific help
            column with the current interpretation weight.
            """
            episode_return = np.asarray(episode_return, dtype=np.float32)
            scalar_weight = np.asarray(scalar_weight, dtype=np.float32)
            ...
            if episode_return.size == self.reward_dim and self.reward_dim == self.num_interp * self.net_reward_dim:
                ...
                return float(np.dot(projected_return, scalar_weight))

            raise ValueError(
                f"Cannot scalarize episode return of shape {episode_return.shape} with weight shape {scalar_weight.shape}"
            )
    ...
    def train(
        ...
        # GPI-LS / CCS operate in the net_reward_dim objective space. Evaluation must
        # see the env reward projected into that space, so wrap eval_env at the fixed
        # eval interp weight.
