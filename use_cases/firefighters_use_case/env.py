...
class HighRiseFireEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.state_space = spaces.MultiDiscrete(
            [1, 5, 5, 2, 2, 4], dtype=np.int64
        )  # Floor, Fire Intensity, Occupancy, Equipment, Visibility, Medical Condition
        """
    ...
    def transition(self, state, action):
        ...
        floor, fire_intensity, occupancy, equipment, visibility, medical_condition = (
            state
        )
        ...
        if action == ACTION_EVACUATE_OCCUPANTS:  # Evacuate Occupants
            ...
            action == ACTION_COORDINATE_WITH_OTHER_AGENCIES
    ...
    def calculate_rewards(self, state, action, next_state):
        ...
        if action == ACTION_EVACUATE_OCCUPANTS:  # Evacuate Occupants
            professionalism_reward = max(
                0, 1 - 0.2 * state[STATE_FIRE_INTENSITY] - 0.1 * state[STATE_VISIBILITY]
            )  # VISIBILITY IS WRONG, BLATANTLY WRONG: IF HIGH VISIBILITY, LOWER PROFESSIONALISM!!!!!!!!
            ...
            professionalism_reward = (
                0.3 if state[STATE_EQUIPMENT] == 0 else 0.6
            )  # Less if equipment degraded
