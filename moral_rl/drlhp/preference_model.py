...
class PreferenceModelConv(nn.Module):
    def __init__(self, state_shape, in_channels=6, n_actions=9):
        ...
        self.action_embedding = nn.Linear(
            n_actions, in_channels * state_shape[0] * state_shape[1]
        )
        self.reward_conv1 = nn.Conv2d(
            in_channels=self.in_channels * 2, out_channels=128, kernel_size=2
        )
    ...
    def forward(self, state, action):
        action_embedded = (
            F.one_hot(torch.tensor(action).long(), self.n_actions).float().to(device)
        )
        action_embedded = self.relu(self.action_embedding(action_embedded)).view(
...
class PreferenceModelMLP(nn.Module):
    def __init__(self, state_shape, in_channels=6, n_actions=9):
        ...
        self.reward_l1 = nn.Linear(
            self.in_channels * self.state_shape[0] * self.state_shape[1], 512
        )
...
def update_preference_model(
    preference_model, preference_buffer, preference_optimizer, batch_size
):
    overall_loss = torch.tensor(0.0).to(device)
