...
if __name__ == "__main__":
    ...
    vec_env = SubprocVecEnv(
        [make_env(config.env_id, i) for i in range(config.n_workers)]
    )
    ...
    preference_model = PreferenceModelConv(
    ...
    preference_optimizer = torch.optim.Adam(
        preference_model.parameters(), lr=config.lr_reward
    )
    ...
    for t in tqdm(range(int(config.env_steps / config.n_workers))):
        ...
        preference_rewards = preference_model.forward(
            preference_state, actions
        ).squeeze(1)
        preference_rewards = list(preference_rewards.detach().cpu().numpy())

        train_ready = dataset.write_tuple(
            states, actions, preference_rewards, done, log_probs, rewards
        )
        ...
        if train_ready:
            ...
            rand_idx = np.random.choice(
                range(len(dataset.trajectories)), 2, replace=False
            )
        ...
        if (
            t % config.update_reward_freq == 0
            and t > config.pretrain
            and len(preference_buffer.storage) > config.preference_warmup
        ):
            # Update preference model
            preference_loss = update_preference_model(
                preference_model,
                preference_buffer,
