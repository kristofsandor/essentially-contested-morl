    ...
    def update(self, actions, board, layers, backdrop, things, the_plot):
        ...
        if the_plot.frame == 0:
            ...
            random_positions = np.random.choice(
                14 * 14, size=N_MAIL + N_CITIZEN + N_STREET + N_VASE + 1, replace=False
            )
            ...
            the_plot["C_pos"] = [
                scalar_to_idx(i) for i in random_positions[N_MAIL : N_MAIL + N_CITIZEN]
            ]
            the_plot["S_pos"] = [
                scalar_to_idx(i)
                for i in random_positions[
                    N_MAIL + N_CITIZEN : N_MAIL + N_CITIZEN + N_STREET
                ]
            ]
            the_plot["V_pos"] = [
                scalar_to_idx(i)
