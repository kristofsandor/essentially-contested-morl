...
class DiscriminatorMLP(nn.Module):
    def __init__(self, state_shape, in_channels=6):
        ...
        self.reward_l1 = nn.Linear(
            self.in_channels * self.state_shape[0] * self.state_shape[1], 256
        )
        self.reward_l2 = nn.Linear(256, 512)
        self.reward_l3 = nn.Linear(512, 256)
        self.reward_out = nn.Linear(256, 1)

        self.value_l1 = nn.Linear(
            self.in_channels * self.state_shape[0] * self.state_shape[1], 256
        )
...
class Discriminator(nn.Module):
    def __init__(self, state_shape, in_channels=6, latent_dim=None):
        ...
        if latent_dim is not None:
            ...
            self.latent_embedding_value = nn.Linear(
                latent_dim, state_shape[0] * state_shape[1]
            )
            self.latent_embedding_reward = nn.Linear(
                latent_dim, state_shape[0] * state_shape[1]
            )
        ...
        # Layers
        # self.action_embedding = nn.Linear(n_actions, state_shape[0]*state_shape[1])
        self.reward_conv1 = nn.Conv2d(
            in_channels=self.in_channels, out_channels=32, kernel_size=2
        )
    ...
    def g(self, state, latent=None):
        state = state.view(
            -1, self.in_channels, self.state_shape[0], self.state_shape[1]
        )
...
def training_sampler(
    expert_trajectories, policy_trajectories, ppo, batch_size, latent_posterior=None
):
    states = []
    ...
    for i in range(batch_size):
        ...
        if latent_posterior is not None:
            if expert_boolean == 1:
                ...
                latent = torch.tensor(
                    selected_trajectories[random_tau_idx]["latents"]
                ).to(device)

            action_probability, _ = ppo.forward(
                torch.tensor(state).float().to(device), latent
            )
        ...
        selected_action = selected_trajectories[random_tau_idx]["actions"][
            random_state_idx
        ]
...
def update_discriminator(
    ...
    class_predictions = torch.cat(
        [torch.log(action_probabilities).unsqueeze(1), advantages], dim=1
    )
    ...
    return (
        loss.item(),
        torch.mean(predicted_fake).item(),
        torch.mean(predicted_expert).item(),
    )
