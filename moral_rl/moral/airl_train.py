...
if __name__ == "__main__":
    ...
    expert_trajectories = pickle.load(
        open("../demonstrations/ppo_demos_v3_[0,1,0,1].pk", "rb")
    )
    ...
    # Initialize Models
    ppo = PPO(state_shape=state_shape, n_actions=n_actions, in_channels=in_channels).to(
        device
    )
    ...
    for t in tqdm(range((int(config.env_steps / config.n_workers)))):
        ...
        airl_rewards = discriminator.predict_reward(
            airl_state, airl_next_state, config.gamma, airl_action_prob
        )
        airl_rewards = list(
            airl_rewards.detach().cpu().numpy() * [0 if i else 1 for i in done]
        )

        # Save Trajectory
        train_ready = dataset.write_tuple(
            states, actions, airl_rewards, done, log_probs
        )
