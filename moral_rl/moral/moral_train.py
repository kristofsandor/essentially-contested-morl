...
if __name__ == "__main__":
    ...
    ppo = PPO(state_shape=state_shape, in_channels=in_channels, n_actions=n_actions).to(
        device
    )
    ...
    discriminator_0 = Discriminator(
        state_shape=state_shape, in_channels=in_channels
    ).to(device)
    discriminator_0.load_state_dict(
        torch.load("../saved_models/discriminator_v3_[0,1,0,1].pt")
    )
    ppo_0 = PPO(
        state_shape=state_shape, in_channels=in_channels, n_actions=n_actions
    ).to(device)
    ...
    discriminator_1.load_state_dict(
        torch.load("../saved_models/discriminator_v3_[0,0,1,1].pt")
    )
    ppo_1 = PPO(
        state_shape=state_shape, in_channels=in_channels, n_actions=n_actions
    ).to(device)
    ...
    dataset = TrajectoryDataset(
        batch_size=config.batchsize_ppo, n_workers=config.n_workers
    )
    ...
    preference_learner = PreferenceLearner(
        d=len(config.ratio), n_iter=10000, warmup=1000
    )
    ...
    for t in tqdm(range(env_steps)):
        ...
        airl_rewards_0 = discriminator_0.forward(
            airl_state, airl_next_state, config.gamma
        ).squeeze(1)
        airl_rewards_1 = discriminator_1.forward(
            airl_state, airl_next_state, config.gamma
        ).squeeze(1)
        airl_rewards_0 = airl_rewards_0.detach().cpu().numpy() * [
            0 if i else 1 for i in done
        ]
        airl_rewards_1 = airl_rewards_1.detach().cpu().numpy() * [
            0 if i else 1 for i in done
        ]
        vectorized_rewards = [
            [r[0], airl_rewards_0[i], airl_rewards_1[i]] for i, r in enumerate(rewards)
        ]
        scalarized_rewards = [
            np.dot(w_posterior_mean, r[0:3]) for r in vectorized_rewards
        ]
        ...
        train_ready = dataset.write_tuple(
            states, actions, scalarized_rewards, done, log_probs
        )
        ...
        if train_ready:
            ...
            if volume_buffer.auto_pref:
                new_returns_a, new_returns_b, logs_a, logs_b = (
                    volume_buffer.sample_return_pair()
                )
                volume_buffer.compare_delta(
                    w_posterior,
                    new_returns_a,
                    new_returns_b,
                    logs_a,
