...
from env.tabularVAenv import TabularVAMDP, encrypt_state, grounding_func_from_matrix
...
class FireFightersEnv(TabularVAMDP):
    ...
    def __init__(
        self,
        env_name="FireFighters-v0",
        feature_selection=FeatureSelectionFFEnv.ONE_HOT_FEATURES,
        horizon=100,
        initial_state_distribution="good_state",
    ):
        ...
        for s in range(self.n_states):
            ...
            if feature_selection == FeatureSelectionFFEnv.ORIGINAL_OBSERVATIONS:
                ...
                vec = np.concatenate(
                    [
                        np.eye(self.real_env.state_space.nvec[i])[s_trans[i]]
        ...
        super(FireFightersEnv, self).__init__(
            env_name=env_name,
            n_values=2,
            transition_matrix=transition_matrix,
            observation_matrix=observation_matrix,
            reward_matrix_per_va=self._get_reward_matrix_per_va,
            default_reward_matrix=reward_matrix_per_va[(0.0, 1.0)],
            horizon=horizon,
