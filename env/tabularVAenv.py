...
from env.mce_partition import mce_partition_fh
...
class ValueAlignedEnvironment(gym.Env):
    ...
    def compute_policy(
        env,
        reward: Callable[
            [Optional[Any], Optional[Any], Optional[Any], Optional[Any]], float
        ],
        discount=1.0,
        **kwargs,
    ):
    ...
    def __init__(
        ...
        n_values: int,
        horizon: int = None,
        done_when_horizon_is_met: bool = False,
        trunc_when_horizon_is_met: bool = True,
    ):
        ...
        self.basic_profiles = [
            tuple([float(ti) for ti in t]) for t in np.eye(self.n_values)
        ]
    ...
    def get_reward_per_value(
        ...
        return self.get_reward_per_align_func(
            align_func=self.basic_profiles[vindex],
            state=state,
            action=action,
    ...
    def step(
        ...
        r = self.get_reward_per_align_func(
            align_func=info["align_func"],
            state=self.prev_observation,
            action=action,
            next_state=ns,
            done=done,
            info=info,
            custom_grounding=self.get_grounding_func(),
        )
        self.time += 1
...
class TabularVAMDP(ValueAlignedEnvironment):
    ...
    def __init__(
        ...
        default_reward_matrix: ndarray,
        initial_state_dist: ndarray = None,
        horizon: int = None,
        done_when_horizon_is_met: bool = False,
        trunc_when_horizon_is_met: bool = True,
    ):
        ...
        #   OR n_states
        if (
            default_reward_matrix.shape
            != transition_matrix.shape[: len(default_reward_matrix.shape)]
        ):
            raise ValueError(
    ...
    def get_reward_per_align_func(
        ...
        return self.reward_matrix_per_align_func(
            align_func, custom_grounding=custom_grounding
        )[
            (
                info["state"]
                if state is None or isinstance(state, Iterable)
                else int(state)
            ),
            action,
        ]
...
def reward_func_from_matrix_partial(
    reward_matrix: np.ndarray,
    state=None,
    action=None,
    next_state=None,
    ...
    if len(reward_matrix.shape) == 2:
        if action is not None:
            ...
            return (
                reward_matrix[state, next_state]
                if state is not None and next_state is not None
                else (
                    reward_matrix[state, :]
                    if state is not None
                    else (
                        reward_matrix[:, next_state]
        ...
        return (
            reward_matrix[state, action, next_state]
            if state is not None and action is not None and next_state is not None
                ...
                if state is not None and action is not None
                    ...
                    if state is not None and next_state is not None
                        ...
                        reward_matrix[:, action, next_state]
        ...
        if done is not None:
            return (
                reward_matrix[state, action, next_state, done]
                if state is not None and action is not None and next_state is not None
                else (
                    reward_matrix[state, action, :, done]
                    if state is not None and action is not None
                    else (
                        reward_matrix[state, :, next_state, done]
                        if state is not None and next_state is not None
                        else (
                            reward_matrix[:, action, next_state, done]
                            if action is not None and next_state is not None
                            else (
                                reward_matrix[state, :, :, done]
                                if state is not None
                                else (
                                    reward_matrix[:, action, :, done]
                                    if action is not None
                                    else (
                                        reward_matrix[:, :, next_state, done]
                                        if next_state is not None
                                        else reward_matrix[..., done]
                                    )
                                )
                            )
                        )
...
def grounding_func_from_matrix_partial(
    assumed_grounding: np.ndarray,
    vi,
    state=None,
    action=None,
    next_state=None,
    ...
    if len(assumed_grounding.shape) == 3:
        ...
        if action is not None:
            ret = (
                assumed_grounding[state, action, :]
                if (state is not None) and (action is not None)
                else (
                    assumed_grounding[state, :, :]
                    if state is not None
                    else (
                        assumed_grounding[:, action, :]
                        if action is not None
                        else assumed_grounding
                    )
                )
            )
        else:
            ret = (
                assumed_grounding[state, next_state, :]
                if state is not None and next_state is not None
                else (
                    assumed_grounding[state, :, :]
                    if state is not None
                    else (
                        assumed_grounding[:, next_state, :]
                        if next_state is not None
                        else assumed_grounding
                    )
                )
        ...
        ret = (
            assumed_grounding[state, action, next_state, :]
            if (state is not None) and (action is not None) and (next_state is not None)
            else (
                assumed_grounding[state, action, :, :]
                if (state is not None) and (action is not None)
                    ...
                    if (state is not None) and (next_state is not None)
                        ...
                        assumed_grounding[:, action, next_state, :]
                        if (action is not None) and (next_state is not None)
                        else (
                            assumed_grounding[state, :, :, :, :]
                            if (state is not None)
        ...
        if done is not None:
            ret = (
                assumed_grounding[state, action, next_state, :, done, :]
                if (state is not None)
                and (action is not None)
                and (next_state is not None)
                else (
                    assumed_grounding[state, action, :, done, :]
                    if state is not None and action is not None
                    else (
                        assumed_grounding[state, :, next_state, done, :]
                        if state is not None and next_state is not None
                        else (
                            assumed_grounding[:, action, next_state, done, :]
                            if action is not None and next_state is not None
                            else (
                                assumed_grounding[state, :, :, done, :]
                                if state is not None
                                else (
                                    assumed_grounding[:, action, :, done, :]
                                    if action is not None
                                    else (
                                        assumed_grounding[:, :, next_state, done, :]
                                        if next_state is not None
                                        else assumed_grounding[..., done]
                                    )
                                )
                            )
                        )
