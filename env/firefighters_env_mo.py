...
from env.tabularVAenv import encrypt_state, grounding_func_from_matrix
...
class FireFightersEnvMO(gym.Env):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.reward_space = spaces.Box(
            low=-50.0, high=50.0, shape=(self.n_values,), dtype=np.float32
        )
        self.reward_dim = (
            self.n_values
        ...
        for s in range(self.n_states):
            ...
            if feature_selection == FeatureSelectionFFEnv.ORIGINAL_OBSERVATIONS:
                ...
                observation_matrix[s, :] = np.eye(
                    self.real_env.n_states, dtype=observation_matrix.dtype
                )[s]
            elif feature_selection == FeatureSelectionFFEnv.ENCRYPTED_OBSERVATIONS:
                ...
                vec = np.concatenate(
                    [
                        np.eye(
                            self.real_env.state_space.nvec[i],
                            dtype=observation_matrix.dtype,
                        )[s_trans[i]]
                        for i in range(self.real_env.state_space.nvec.shape[0])
                    ],
                    -1,
                )
            ...
            if not self.real_env.is_done(s_trans):
                for a in range(self.action_space.n):
                    ...
                    (
                        reward_matrix_per_va[(1.0, 0.0)][s, a],
                        reward_matrix_per_va[(0.0, 1.0)][s, a],
                    ) = self.real_env.calculate_rewards(s_trans, a, ns_trans)
            else:
        ...
        if isinstance(initial_state_distribution, np.ndarray):
            ...
            or initial_state_distribution == "random"
        ):
            ...
            self.initial_state_dist[
                self.real_env.encrypt(np.array([0, 3, 4, 0, 0, 3]))
            ] = 1.0
        ...
        self.reward_matrix = np.stack(
            [reward_matrix_per_va[(1.0, 0.0)], reward_matrix_per_va[(0.0, 1.0)]],
    ...
    def _get_reward_matrix_per_va(self, align_func, custom_grounding=None):
        ...
        if custom_grounding is None:
            v = (
                self.reward_matrix_per_va_dict[(1.0, 0.0)] * align_func[0]
                + self.reward_matrix_per_va_dict[(0.0, 1.0)] * align_func[1]
            )
        else:
            ...
            if custom_grounding.shape == (400, 5, 2):
                v = (
                    custom_grounding[:, :, 0] * align_func[0]
        ...
        **kwargs,
        ...
        if self.feature_selection == FeatureSelectionFFEnv.ONE_HOT_OBSERVATIONS:
            ...
            assumed_grounding[:, 0] = np.reshape(
                self.obtain_grounding(
                    variant=variants[0] if variants is not None else None,
                    file_save=(
                        os.path.join(save_folder, variants_save_files[0])
                        if variants_save_files is not None
                        else None
                    ),
            ...
            assumed_grounding[:, 1] = np.reshape(
                self.obtain_grounding(
                    variant=variants[1] if variants is not None else None,
                    file_save=(
                        os.path.join(save_folder, variants_save_files[1])
                        if variants_save_files is not None
                        else None
                    ),
                    recalculate=recalculate,
            ...
            assumed_grounding[:, :, 0] = self.obtain_grounding(
                variant=variants[0] if variants is not None else None,
                file_save=(
                    os.path.join(save_folder, variants_save_files[0])
                    if variants_save_files is not None
            ...
            assumed_grounding[:, :, 1] = self.obtain_grounding(
                variant=variants[1] if variants is not None else None,
                file_save=(
                    os.path.join(save_folder, variants_save_files[1])
                    if variants_save_files is not None
    ...
    def obtain_grounding(self, variant=None, file_save=None, recalculate=True):
        if variant is None or "default" in variant:
            return self.reward_matrix_per_align_func(
                (0.0, 1.0)
                if "proximity" in variant
                else (1.0, 0.0) if "professionalism" in variant else None
            )
            ...
            with open(file_save, "wb" if recalculate else "r") as fsave:
                if recalculate or os.stat(file_save).st_size == 0:
                    ...
                    reward_matrix[reward_matrix > 0] = (
                        1.0  # Always go for professionalism
                    )
    ...
    def terminal(self, state: DiscreteSpaceInt, n_actions_taken: int) -> bool:
        ...
        ), "Horizon must be set to check for terminal state."
        return n_actions_taken >= self.horizon or self.real_env.is_done(
            self.real_env.translate(state)
        )
